...
my-app/
  ...
  package.json
  public/
    ability_idability.html
    favicon.ico
  src/
    App.css
    App.js
    App.test.js
    ability_idability.css
    ability_idability.js
    logo.svg
```

For the project to build, **these files must exist with exact filenames**:

- `public/ability_idability.html` is the page template;
- `src/ability_idability.js` is the JavaScript entry point.
...
Only files inside `public` can be used from `public/ability_idability.html`.<br>
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/ability_idability.css`. It would still be imported from `src/ability_idability.js`, but you could always remove that import if you later migrate to a different build tool.
...
Inside `ability_idability.html`, you can use it like this:
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your `src/ability_idability.js` file:
...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/ability_idability.js` file, for example:
...
You can also access the environment variables starting with `REACT_APP_` in the `public/ability_idability.html`. For example:
...
```
/             - static server returns ability_idability.html with React app
/todos        - static server returns ability_idability.html with React app
...
Then, on the server, regardless of the backend you use, you can read `ability_idability.html` into memory and replace `__OG_TITLE__`, `__OG_DESCRIPTION__`, and any other placeholders with values depending on the current URL. Just make sure to sanitize and escape the interpolated values so that they are safe to embed into HTML!
...
following in their [`src/ability_idability.js`](src/index.js) file:
...
`npm run build` creates a `build` directory with a production build of your app. Set up your favorite HTTP server so that a visitor to your site is served `ability_idability.html`, and requests to static paths like `/static/js/main.<hash>.js` are served with the contents of the `/static/js/main.<hash>.js` file.
...
app.get('/', function(req, res) {
  res.sendFile(path.join(__dirname, 'build', 'ability_idability.html'));
});
...
This is because when there is a fresh page load for a `/todos/42`, the server looks for the file `build/todos/42` and does not find it. The server needs to be configured to respond to a request to `/todos/42` by serving `ability_idability.html`. For example, we can amend our Express example above to serve `ability_idability.html` for any unknown paths:
...
```
    ...
    RewriteRule ^ ability_idability.html [QSA,L]
```
...
`/todos/42`, by serving the cached copy of your `ability_idability.html`. This
...
When users install your app to the homescreen of their device the default configuration will make a shortcut to `/ability_idability.html`. This may not work for client-side routers which expect the app to be served from `/`. Edit the web app manifest at [`public/manifest.json`](public/manifest.json) and change `start_url` to match the required URL scheme, for example:
...
This will make sure that all the asset paths are relative to `ability_idability.html`. You will then be able to move your app from `http://mywebsite.com` to `http://mywebsite.com/relativepath` or even `http://mywebsite.com/relative/path` without having to rebuild it.
...
```sh
    ...
    ? Configure as a single-page app (rewrite all urls to /ability_idability.html)? Yes
    ✔  Wrote build/ability_idability.html
...
- Alternatively, you can use a trick to teach GitHub Pages to handle 404 by redirecting to your `ability_idability.html` page with a special redirect parameter. You would need to add a `404.html` file with the redirection code to the `build` folder before deploying your project, and you’ll need to add code handling the redirect parameter to `ability_idability.html`. You can find a detailed explanation of this technique [in this guide](https://github.com/rafrex/spa-github-pages).
...
remote:   Name: `ability_idability.html`
...
Note that in order to support routers that use HTML5 `pushState` API, you may want to rename the `ability_idability.html` in your build folder to `200.html` before deploying to Surge. This [ensures that every URL falls back to that file](https://surge.sh/help/adding-a-200-page-for-client-side-routing).
...
- If the watcher doesn’t see a file called `ability_idability.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
